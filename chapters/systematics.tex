...
\section{Light Model}
To estimate the uncertainty on the light model a three special datasets are generated. The first dataset which is called
nominal is generated with increased Cerenkov light collection efficiency and reduced standard scintillation light produciton 
effectively removing $\frac{dE}{dx}$ discrepancy observed in the ratio of ND data/simulation for muons and protons. The 
second and the third datasets are regular $\pm 1\sigma$ shifts where alternative scintillator light levels are shifted by 
10\% up and down while callibration simulteneously adjusted by 10\% in the opposite direction. The effect of the shifts is
that some hits falling below or rising above the readout treshold.

In comparison to contained sample light model systematic has a significant effect on escaping sample\footnote{a small effect 
...
sensetive to the light model. As result, $\pm 1\sigma$ shifts could significantly affect those hits thereby making an event
...
{"Nominal" column shows number of selected events for nominal simulations, columns "Shift ($\pm$)"
show number selected events when light level shifted sample is used, columns "Diff ($\pm$)" show the same shifts
in a relative manner.}
\end{table} }
